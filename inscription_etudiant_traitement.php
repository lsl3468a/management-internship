<?php
    ...
    if(!empty($_POST['nom_etudiant']) && !empty($_POST['prenom_etudiant']) && !empty($_POST['num_etudiant']) && !empty($_POST['mail_etudiant']) && !empty($_POST['mdp_etudiant']) && !empty($_POST['retype_mdp']))
    {
        ...
        $retype_mdp = htmlspecialchars($_POST['retype_mdp']);


        $check = $bdd->prepare('SELECT nom_etudiant, prenom_etudiant, num_etudiant, mail_etudiant, mdp_etudiant, inscription_alerte FROM etudiant WHERE mail = ?');
                    ...
                    if(strlen($num_etudiant) <= 8){
                        if(strlen($mail_etudiant) <= 50){
                            if(filter_var($mail_etudiant, FILTER_VALIDATE_EMAIL)){
                                if($mdp_etudiant == $retype_mdp){

                                  if(isset($_POST["inscription_alerte"])){
                                ...
                                $insert = $bdd->prepare('INSERT INTO `etudiant`(`num_etudiant`, `nom_etudiant`, `prenom_etudiant`, `mdp_etudiant`, `mail_etudiant`, `inscription_alerte`) VALUES (:nom_etudiant, :prenom_etudiant, :num_etudiant, :mail_etudiant, :mdp_etudiant, :inscription_alerte)');
