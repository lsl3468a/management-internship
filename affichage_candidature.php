...
<body>
	...
	<div id="content">
			<table align="center">
					...
					<tbody>
							   <?php
								$req= "SELECT id_stage from stage WHERE id_contact ='".$_SESSION["id_contact"]."'";
								$result = $conn->query($req);
								while($row = $result->fetch_assoc()){
									$req2 = "SELECT * from candidater WHERE id_stage ='".$row["id_stage"]."'";
									$result2 = $conn->query($req2);
									while($row2 = $result2->fetch_assoc()){
										?>
									<tr>
									<td><?php echo $row2['id_stage']?></td>
									<td><?php echo $row2['num_etudiant']?></td>
									<td><a href="<?php echo utf8_encode($row2['CV'])?>"> CV </a></td>
									<td><a href="<?php echo utf8_encode($row2['LM'])?>"> LM </a></td>
									<td><?php echo utf8_encode($row2['commentaires'])?></td>	
									</tr>
									 <?php
									}
								}
