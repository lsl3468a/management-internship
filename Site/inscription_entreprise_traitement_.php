                                                ...
                                                if(strlen($mail_contact) <= 50){
                                                    if(filter_var($mail_contact, FILTER_VALIDATE_EMAIL)){
                                                        if($mdp_contact == $retape_mdp){
                                                             ...
                                                             $insert = $bdd->prepare('INSERT INTO `contact`(`nom`, `id_entreprise`, `prenom`, `mail_contact`,`tel_contact`, `mdp_contact`) VALUES (?,?,?,?,?,md5(?))');
                                                             $insert->execute(array($nom, $data['id_entreprise'], $prenom, $mail_contact, $tel_contact, $mdp_contact));


                                                             header('Location:accueil.php?reg_err=success');
                                                             die();
                                                       }else{ header('Location: inscrip.php?reg_err=mdp_contact'); die();}
                                                   }else{ header('Location: inscrip.php?reg_err=mail_contact'); die();}
                                               }else{ header('Location: inscrip.php?reg_err=mail_contact_length'); die();}
                                           }else{ header('Location: inscrip.php?reg_err=tel_contact_length'); die();}
                                        }else{ header('Location: inscrip.php?reg_err=prenom_length'); die();}
                                    }else{ header('Location: inscrip.php?reg_err=nom_length'); die();}
                                }else{ header('Location: inscrip.php?reg_err=num_siret_length'); die();}
                            }else{ header('Location: inscrip.php?reg_err=code_postal_length'); die();}
                        }else{ header('Location: inscrip.php?reg_err=pays_length'); die();}
                    }else{ header('Location: inscript.php?reg_err=ville_length'); die();}
                }else{ header('Location: inscrip.php?reg_err=adresse_length'); die();}
            }else{ header('Location: inscript.php?reg_err=nom_societe_length'); die();}
        }else{ header('Location: inscript.php?reg_err=already'); die();}
    }
